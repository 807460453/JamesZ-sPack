    ...
    <el-tabs v-model="activeTab" type="card" tab-position="top" @tab-click="">
        <el-tab-pane label="demonstration" name="demo">
            <div style="width:500px">
                <DynamicForm ref="cmp_dynamicForm" :items="items" :defaultData="defaultData" :defaultRules="defaultRules"
                    :inline="false">
                    <template #start_form="{ ctx }"> the slot before a form:{{ ctx.test }} </template>
                    <template #after_f4_slot> you can alse add slot before or after a formItem </template>
                    <template #f7_slot="{ ctx }">
                        <span v-if="ctx.form">
                            this is a slot of f7, you can see it when child cmp's form has value, click the btn and the
                            value of f1 will be set to 2 and because there is a if condition that f7 won't be seen
                            <el-button type="primary" class="zy-form-item-btn" @click="textBtnClick(ctx)">test</el-button>
                        </span>
                    </template>
                </DynamicForm>
                <el-button type="primary" size="default" @click="getFormData">getFormData</el-button>

            </div>
        </el-tab-pane>
        <el-tab-pane label="readme" name="md">
...
function initMdString() {
    const proStr = import('./dynamicForm.md?raw')
    proStr.then(_module => {
